    ...
    static class Segmentation
    {
        ...
        public static List<string> SegMM(string inputStr, ref List<string> wordList, bool leftToRight, int maxLength)
        {
            ...
            while (segLength < inputStr.Length)
            {
                ...
                if (System.Text.UTF8Encoding.UTF8.GetBytes(word[0].ToString()).Length == 1)
                {
                    if (leftToRight)
                    {
                        segWords.Add(word[0].ToString());
                    }
                    else
                    {
                        segWordsReverse.Add(word[0].ToString());
                    }
                ...
                else
                {
                    ...
                    while (!wordList.Contains(word))
                    {
                        ...
                        word = leftToRight ? word.Substring(0, word.Length - 1) : word.Substring(1);
                    }
