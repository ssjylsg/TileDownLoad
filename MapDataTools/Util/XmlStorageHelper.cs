namespace MapDataTools.Util
{
    ...
    public class XmlStorageHelper
    {
            ...
            Assembly LoadedAssembly
            {
                get
                {
                    if (!this.isAssemblyLoaded)
                    {
                        ...
                        try
                        {
                            this.loadedAssembly = Assembly.Load(this.AssemblyName);
                            if (this.loadedAssembly == null)
                            {
                                string exePath = AppDomain.CurrentDomain.SetupInformation.ApplicationBase;
                                string assemblyFile = Path.Combine(exePath, this.AssemblyName);
                                if (File.Exists(assemblyFile))
                                    this.loadedAssembly = Assembly.LoadFile(assemblyFile);
                            }
                        }
                        catch
                        {
        ...
        {
            foreach (Runtime rt in this.runtimeList)
            {
        ...
        void SetCollectionToXml(ICollection colObject, XmlElement parentEle)
        {
            ...
            foreach (object o in colObject)
            {
                if (o == null)
                    continue;
                if (this.IsBasicType(o.GetType()))
                {
                    this.AddRuntime(o.GetType().FullName, o.GetType().Assembly.GetName().Name);
                    string attValue = o.ToString();
                    XmlElement subEle = this.xmldoc.CreateElement(o.GetType().Name);
                ...
                {
                    this.SetValueToXml(o, parentEle);
                }
            }
        }
        ...
        void SetDictionaryToXml(IDictionary dictObject, XmlElement parentEle)
        {
            ...
            {
                XmlElement entryEle = this.xmldoc.CreateElement(entry.GetType().Name);
                if (this.IsBasicType(entry.Key.GetType()))
                {
                    this.AddRuntime(entry.Key.GetType().FullName, entry.Key.GetType().Assembly.GetName().Name);
                    XmlElement keyEle = this.xmldoc.CreateElement(entry.Key.GetType().Name);
                    keyEle.SetAttribute("key", entry.Key.ToString());
                    entryEle.AppendChild(keyEle);
                }
                else
                {
                    this.SetValueToXml(entry.Key, entryEle);
                }
                if (this.IsBasicType(entry.Value.GetType()))
                {
                    this.AddRuntime(entry.Value.GetType().FullName, entry.Value.GetType().Assembly.GetName().Name);
                    XmlElement valueEle = this.xmldoc.CreateElement(entry.Value.GetType().Name);
                    valueEle.SetAttribute("value", entry.Value.ToString());
                    entryEle.AppendChild(valueEle);
                }
                else
                {
                    this.SetValueToXml(entry.Value, entryEle);
                }
        ...
        {
            this.SetValueToXml(xmlObject, parentEle, null);
        }
        ...
        void SetValueToXml(object xmlObject, XmlElement parentEle, string tagName)
        {
            ...
            if (this.IsCollection(objType))
            {
                ...
                if (this.IsDictionary(objType))
                {
                    classEle = this.xmldoc.CreateElement(Dictionary_Tag);
                    classEle.SetAttribute("TypeName", typeString);
                    this.SetDictionaryToXml((IDictionary)xmlObject, classEle);
                }
                else
                {
                    classEle = this.xmldoc.CreateElement(Collection_Tag);
                    classEle.SetAttribute("TypeName", typeString);
                    this.SetCollectionToXml((ICollection)xmlObject, classEle);
                }
            }
            else
            {
                if (string.IsNullOrEmpty(tagName))
                    classEle = this.xmldoc.CreateElement(objType.Name);
                else
                    classEle = this.xmldoc.CreateElement(tagName);
                this.AddRuntime(objType.FullName, objType.Assembly.GetName().Name);
                foreach (FieldInfo field in objType.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.GetProperty))
                {
                    ...
                    //如果是集合类型，则需要获取下级
                    if (this.IsCollection(field.FieldType))
                    {
                        XmlElement colEle = this.xmldoc.CreateElement(field.Name);
                        if (this.IsDictionary(field.FieldType))  //是否为字典
                        {
                            this.SetDictionaryToXml((IDictionary)objValue, colEle);
                        }
                        else
                        {
                            this.SetCollectionToXml((ICollection)objValue, colEle);
                        }
                        classEle.AppendChild(colEle);
                    }
                    else
                    {
                        if (this.IsBasicType(field.FieldType))
                        {
                            string attValue = "";
                            if (objValue != null)
                        ...
                        {
                            this.SetValueToXml(objValue, classEle, field.Name);
                        }
                    }
                }
            }
            if (parentEle != null)
                ...
                this.xmldoc.AppendChild(classEle);
        }
        ...
        void GetCollectionFromXml(IList colObject, XmlNode colNode)
        {
            ...
            foreach (XmlNode eleNode in colNode.ChildNodes)
            {
                ...
                {
                    eleType = this.TryToGetType(eleNode.Name);
                }
                if (eleType == null)
                    continue;
                if (this.IsBasicType(eleType))
                {
                ...
                else
                {
                    ...
                    this.GetValueFromXml(o, eleNode);
                    colObject.Add(o);
                }
            }
        }
        ...
        void GetDictionaryFromXml(IDictionary dictObject, XmlNode dictNode)
        {
            ...
            foreach (XmlNode kvNode in dictNode.ChildNodes)
            {
                if (kvNode.Name.Equals("DictionaryEntry"))
                {
                    ...
                    {
                        keyType = this.TryToGetType(keyNode.Name);
                    }
                    if (keyType == null)
                        continue;
                    if (this.IsBasicType(keyType))
                    {
                    ...
                    else
                    {
                        ...
                        this.GetValueFromXml(entryKey, keyNode);
                    }
                    ...
                    if (valueNode.Name.Equals(Collection_Tag) || valueNode.Name.Equals(Dictionary_Tag))
                    {
                        ...
                        valueType = this.TryToGetType(valueNode.Name);
                    if (valueType == null)
                        continue;
                    if (this.IsBasicType(valueType))
                    {
                    ...
                    else
                    {
                        ...
                        this.GetValueFromXml(entryValue, valueNode);
                    }
        ...
        {
            this.GetValueFromXml(xmlObject, objectNode, null);
        }
        ...
        void GetValueFromXml(object xmlObject, XmlNode objectNode, string eleName)
        {
            ...
            {
                this.GetCollectionFromXml(xmlObject as IList, objectNode);
            }
            else if (objectNode.Name.Equals(Dictionary_Tag)) //字典
            {
                this.GetDictionaryFromXml(xmlObject as IDictionary, objectNode);
            }
            else
            {
                Type objType = xmlObject.GetType();
                ...
                if (objectNode.Name.ToLower().Equals(tagName.ToLower()))
                {
                    ...
                    foreach (FieldInfo field in objType.GetFields(BindingFlags.Public | BindingFlags.Instance))
                    {
                        ...
                        if (this.IsCollection(field.FieldType))
                        {
                            foreach (XmlNode node in objectNode.ChildNodes)
                            {
                                if (node.Name.Equals(field.Name))
                                {
                                    ...
                                    if (this.IsDictionary(field.FieldType))
                                    {
                                        this.GetDictionaryFromXml(colObject as IDictionary, node);
                                    }
                                    else
                                    {
                                        this.GetCollectionFromXml(colObject as IList, node);
                                    }
                        ...
                        {
                            if (this.IsBasicType(field.FieldType)) //简单内置类型
                            {
                            ...
                            else  //自定义类型
                            {
                                foreach (XmlNode node in objectNode.ChildNodes)
                                {
                                    if (node.Name.Equals(field.Name))
                                    {
                                        ...
                                        this.GetValueFromXml(o, node, field.Name);
        ...
        Type TryToGetType(string typeName)
        {
            ...
            if (t != null)
                return t;
            foreach (Runtime r in this.runtimeList)
            {
        ...
        public bool LoadFromFile(object xmlObject, string xmlPath)
        {
            ...
            try
            {
                ...
                else if (this.IsCollection(objType))
